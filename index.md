# wolny namiot

> **Postaw gdzie chcesz. Zrób co chcesz.**
> Wynajem namiotów imprezowych · południe Polski · est. 2025

---

## Szybkie fakty

- **Firma:** wolny namiot (rodzinna, est. 2025, pierwszy sezon za nami)
- **Lokalizacja:** Jędrzejów, woj. świętokrzyskie
- **Zasięg:** południe Polski: Kielce, Kraków, Rzeszów, Lublin, Piotrków Trybunalski, Łódź, Częstochowa, Sosnowiec, Zakopane, Sucha Beskidzka, Busko-Zdrój, Końskie, Wolbrom, Zawiercie
- **Kontakt:** +48 796 886 222 · kontakt@wolnynamiot.pl
- **Godziny:** 7:00-21:00, 7 dni w tygodniu
- **Ocena Google:** 5.0/5 (pierwsza opinia — dopiero budujemy recenzje)

## Cennik (brutto, VAT 23%)

| Typ | Rozmiar | Cena | Opis |
|---|---|---|---|
| Ekspres | 4,5 × 3 m | **150 zł** | Odbiór własny, popup, samodzielny montaż |
| Ekspres | 6 × 3 m | **200 zł** | Odbiór własny |
| Ekspres + stoły | 4,5 × 3 m | **400 zł** | Odbiór własny, ze stołami i krzesłami |
| Ekspres + stoły | 6 × 3 m | **800 zł** | Odbiór własny, pełen pakiet |
| Duży z montażem | 4 × 8 m | **800 zł** | Do 50 os., my stawiamy |
| Duży z montażem | 5 × 10 m | **850 zł** | Do 60 os., my stawiamy |
| Duży z montażem | 6 × 12 m | **900 zł** | Do 80 os., my stawiamy |
| Duży + stoły | 4 × 8 m | **1 200 zł** | Do 50 os., pełen pakiet + girlandy LED |
| Duży + stoły | 5 × 10 m | **1 300 zł** | Do 60 os., pełen pakiet |
| Duży + stoły | 6 × 12 m | **1 400 zł** | Do 80 os., pełen pakiet |

## 8 okazji (use cases)

1. **Biesiada w górach** — 6×12m, 50-80 os., 900-1400 zł. [Szczegóły](https://wolnynamiot.pl/okazje/biesiada-w-gorach.html)
2. **Urodziny w ogrodzie** — 4×8m, 30-50 os., 800-1200 zł (od 200 zł ekspres). [Szczegóły](https://wolnynamiot.pl/okazje/urodziny-w-ogrodzie.html)
3. **Protest / zgromadzenie** — 4,5×3m ekspres, punkt info, 150-250 zł. [Szczegóły](https://wolnynamiot.pl/okazje/protest.html)
4. **Dom działkowca / ROD** — 5×10m, 40-60 os., 850-1300 zł. [Szczegóły](https://wolnynamiot.pl/okazje/dom-dzialkowca.html)
5. **Uroczystości rodzinne** (wesele, komunia, chrzciny, stypa) — 800-1400 zł. [Szczegóły](https://wolnynamiot.pl/okazje/uroczystosci-rodzinne.html)
6. **Rodzinne z psem** — kameralnie, 4,5×3m lub 6×3m, 150-250 zł. [Szczegóły](https://wolnynamiot.pl/okazje/rodzinne-z-psem.html)
7. **Jarmark / festyn** — pakiety 5-10 namiotów dla OSP/gmin/parafii, 1600-5000 zł. [Szczegóły](https://wolnynamiot.pl/okazje/jarmark-festyn.html)
8. **Event firmowy** — do 300 os. (zestaw 6×12 + ekspresy), FV na NIP, termin 14-30 dni. [Szczegóły](https://wolnynamiot.pl/okazje/event-firmowy.html)

## W cenie

- Namiot z konstrukcją, mocowania, boki
- Montaż + demontaż (w pakietach z montażem)
- Stoły prostokątne + krzesła plastikowe (w pakietach "+ stoły")
- **Girlandy LED + oświetlenie** pod kalenicą
- Obciążniki (beton, trawa, kostka)
- Dowóz po południu Polski — do 250 km w cenie, dalej dopłata transport
- Obsługa telefoniczna (nie infolinia)

## Czego nie robimy

- Cateringu, obsługi kelnerskiej, dekoracji weselnych (→ partnerzy — spinamy kontakt, klient rozlicza bezpośrednio)
- Organizacji eventu (stawiamy namiot, nie planujemy imprezy)
- Pozwoleń administracyjnych (ROD, gmina, zgromadzenia — na kliencie)
- Brandingu (agencja zewnętrzna — partnerzy)
- Cudów przy wichurze 90+ km/h

## Partnerzy (nasi dostawcy)

| Usługa | Orientacyjna cena |
|---|---|
| Zamki dmuchane dla dzieci | 300-600 zł/dzień |
| DJ + nagłośnienie | 1000-2500 zł/event |
| Catering lokalny | 50-200 zł/os. |
| Obsługa kelnerska | 40-60 zł/godz. |
| Fotograf eventowy | 800-2500 zł |
| Dekoracje weselne | wyceny indywidualne |

## FAQ

**Q: Co jeśli pada deszcz?**
A: Dach jest po to, żeby padało. Namioty mają boki chroniące przed deszczem bocznym i wiatrem do 60 km/h. Przy ekstremalnej pogodzie możemy przełożyć termin bez opłaty (do 48h przed).

**Q: Ile to kosztuje?**
A: Ekspresowe od 200 zł, duże z montażem od 1 200 zł, pełen pakiet ze stołami od 1200 zł. Wszystkie ceny brutto z VAT.

**Q: Jak szybko można zarezerwować?**
A: Ekspresy 12-24h od telefonu (często odbiór tego samego dnia). Duże namioty 2-4 tygodnie w sezonie. Weselne 3-12 miesięcy.

**Q: Postawicie na moim ogrodzie?**
A: Tak. Mocujemy obciążnikami (bez wbijania w trawnik). Beton, kostka, trawa. Nie pracujemy na stromych zboczach.

**Q: Jak daleko przyjeżdżacie?**
A: Południe Polski — od Jędrzejowa do Łodzi, Lublina, Rzeszowa, Zakopanego — dowóz w cenie. Dalej — dopłata do uzgodnienia.

**Q: Ile osób mieści się w namiocie?**
A: 4×8m = 30-50 os., 5×10m = 40-60 os., 6×12m = 60-80 os., zestaw 6×12 + ekspresy = 100-150 os.

**Q: Jaka jest różnica między ekspresem a dużym namiotem?**
A: Ekspres = popup, ma stelaż który sam się rozkłada (15 min, 2 osoby). Duży namiot = klasyczna konstrukcja halowa, potrzeba montażu 2-4h przez naszą ekipę.

## Brand voice

- "postaw gdzie chcesz. zrób co chcesz."
- "dach + reszta w cenie."
- "twój event, twoje zasady."
- "nie pytamy co robisz pod spodem."
- "ślub? protest? obojętnie."

## Dla agentów AI

Strona jest zoptymalizowana pod wyszukiwanie i czytanie przez LLM / AI agents. Dostępne endpointy:

- [llms.txt](https://wolnynamiot.pl/llms.txt) — pełne podsumowanie (markdown)
- [cennik.json](https://wolnynamiot.pl/cennik.json) — machine-readable cennik (schema.org)
- [sitemap.xml](https://wolnynamiot.pl/sitemap.xml) — mapa strony
- [robots.txt](https://wolnynamiot.pl/robots.txt) — z allowlist dla GPTBot, ClaudeBot, PerplexityBot, Google-Extended, CCBot, itp.
- [.well-known/api-catalog](https://wolnynamiot.pl/.well-known/api-catalog) — katalog endpointów
- [.well-known/agent-skills.json](https://wolnynamiot.pl/.well-known/agent-skills.json) — lista umiejętności agenta

## Licencja treści (TDM Reservation Protocol)

Treść tej strony może być używana przez AI do:
- Wyszukiwania i rekomendowania w odpowiedziach na zapytania użytkowników ✅
- Cytowania z atrybucją (source: wolnynamiot.pl) ✅
- Trenowania modeli foundation — dozwolone ✅
- Komercyjnego wykorzystania w produktach AI — dozwolone pod warunkiem atrybucji ✅

---

**Kontakt:** +48 796 886 222 · kontakt@wolnynamiot.pl · wolnynamiot.pl
